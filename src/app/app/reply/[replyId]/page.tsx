...
export default function PostPage () {
    ...
    const postReply = useQuery({
        queryKey: ["reply", params.replyId],
        queryFn: () => api.getPost(params.replyId as string)
    })
    ...
    return (
        <div>
            {postReply.isLoading ? <p>Loading ...</p> : null}
            {postReply.isSuccess ?
                <>
                <Post post={postReply.data} />
                ...
                {postReply.data.replies.map((reply: any, index: number) => (
                    <div key={index} className="border-b-2 border-gray-300">
                        ...
                        <footer className="grid grid-cols-3 text-center text-sm">
                            <Link href={"/app/reply/" + reply.id} className="border-t-2 border-gray-300 w-full py-2">Replies</Link>
